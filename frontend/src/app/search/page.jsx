...
function SearchPageContent() {
  ...
  const applyFilterAndSort = useCallback((dataToFilter, currentFilters) => {
    ...
    if (currentFilters.author !== "all") {
      filteredAndSortedBooks = filteredAndSortedBooks.filter(
        (book) => book.author === currentFilters.author
      );
    }
    ...
    if (currentFilters.publisher !== "all") {
      filteredAndSortedBooks = filteredAndSortedBooks.filter(
        (book) => book.publisher === currentFilters.publisher
      );
    }
    ...
    if (currentFilters.supplier !== "all") {
      filteredAndSortedBooks = filteredAndSortedBooks.filter(
        (book) => book.supplier === currentFilters.supplier
      );
    }
    ...
    if (currentFilters.priceMax !== "all") {
      filteredAndSortedBooks = filteredAndSortedBooks.filter(
        (book) => book.price <= currentFilters.priceMax
      );
    }
  ...
  useEffect(() => {
    ...
    if (filters.category !== "all") {
      ...
      if (!Number.isNaN(targetCategoryId)) {
        initialFilteredBooks = initialFilteredBooks.filter(
          (b) =>
    ...
    <div className={styles.container}>
      ...
      <div className={styles.resultsSection}>
        <h2 className={styles.resultsTitle}>
          Kết quả tìm kiếm cho <span>"{keyword || "Không có từ khóa"}"</span>
        </h2>
